...
export default function Profile() {
    ...
    return (
        ...
        <div className="absolute inset-0 bg-[#f8fafc] flex flex-col overflow-hidden font-sans">
                ...
                {purchaseIntent && (
                    <>
                        ...
                        <motion.div initial={{scale: 0.9, opacity: 0, y: "50%", x: "-50%"}}
                                    ...
                                    className="fixed top-1/2 left-1/2 w-[85%] max-w-sm bg-white rounded-3xl p-6 z-[201] shadow-2xl flex flex-col items-center text-center">
                            <div
                            ...
                            <div className="flex gap-3 w-full">
                                ...
                                <button onClick={confirmPurchase} disabled={isSaving}
                                        className="flex-1 bg-beer text-white p-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-beer/30 active:scale-95 transition-transform flex justify-center items-center gap-2 disabled:opacity-50">
                                    {isSaving ? '...' : 'Acheter'} <Check size={16}/>
                                </button>
                            </div>
                        </motion.div>
                    </>
            ...
            {/* 🚀 ZONE PRINCIPALE (Avec le fix du scroll actif) */}
            <div
                className="flex-1 flex flex-col min-h-0 pt-[env(safe-area-inset-top)] overflow-hidden relative z-10 animate-in slide-in-from-right duration-500">

                {/* AVATAR 3D : On ajoute pt-16 pour qu'il commence sous le header compact */}
                <div className="h-[38vh] w-full relative flex-shrink-0 pt-16">
                    <AvatarCanvas config={previewAvatar}/>
                </div>

                {/* LA BOÎTE MAGIQUE POUR LE SCROLL DU VESTIAIRE */}
                <div className="flex-1 flex flex-col min-h-0 w-full overflow-hidden">
                    {isOwnProfile ? (
                        ...
                        <div
                            className="flex-1 flex flex-col min-h-0 bg-white rounded-t-[3rem] shadow-[0_-20px_50px_rgba(0,0,0,0.1)] p-8 overflow-hidden">
